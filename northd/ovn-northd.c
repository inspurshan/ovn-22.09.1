...
main(int argc, char *argv[])
{
    ...
    while (!exiting) {
        ...
        if (northd_probe_interval_nb > 0) {
            VLOG_DBG("Current northd_probe_interval_nb is %d, set poll timer "
                    "wait.", northd_probe_interval_nb);
            poll_timer_wait_until(time_msec() + 2 * northd_probe_interval_nb);
        }
        if (northd_probe_interval_sb > 0) {
            VLOG_DBG("Current northd_probe_interval_sb is %d, set poll timer "
                    "wait.", northd_probe_interval_sb);
            poll_timer_wait_until(time_msec() + 2 * northd_probe_interval_sb);
        }
